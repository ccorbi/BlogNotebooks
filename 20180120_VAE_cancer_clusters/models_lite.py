...
class fusion_VAE(nn.Module):
    def __init__(self, RNA,  miRNA, sub_depth=2, whole_depth=2, rna_decay=5, methy_decay=5, mirna_decay=5, whole_decay=5, cuda=False):
        super(fusion_VAE, self).__init__()

        def layer_construction(in_features, num_layers=2, decay=5):
            layers, sizes = [], []
            for i in range(num_layers):
                tmp_in = int( in_features / (decay ** i) )
                tmp_out = int(  in_features / (decay ** (i + 1)) )
                layers.append(nn.Linear(tmp_in, tmp_out))
                sizes.append(tmp_out)
            return nn.ModuleList(layers), sizes

        self.isCuda= cuda
        rna_size,  mirna_size = RNA.shape[-1],  miRNA.shape[-1]
        ...
        self.sizes = [rna_size +  mirna_size]
        for i in range(sub_depth):
            self.sizes.append(rna_sizes[i] +  mirna_sizes[i])
        ...
        self.hidden_features = int( self.sizes[-1] / whole_decay)
        ...
        command = "Total: %d\tRNA: %d\tmiRNA: %d"
        print( command % (self.sizes[0], rna_size,  mirna_size))
        for i in range(sub_depth-1):
            print(command % (self.sizes[i+1], rna_sizes[i],  mirna_sizes[i]))
    ...
    def encode(self, RNA,  miRNA):
        ...
        tmp = torch.cat((h_rna,  h_mirna), dim=1)
    ...
    def forward(self, RNA, miRNA):
        #mu, logvar = self.encode(RNA, methylation, miRNA)
        mu, logvar = self.encode(RNA,  miRNA)
        z = self.reparametrize(mu, logvar)
        return self.decode(z), mu, logvar
